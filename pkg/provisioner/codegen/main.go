...
func genTerraformCode(src, dst string, exclude ...string) error {
	log.Printf("Generating Terrafrom manifests code")
	...
	defer func() {
		goFile.Close()
		log.Printf("Done with Terraform manifest code")
	}()
...
func getTerraformTemplates(src, dst string, exclude ...string) error {
	...
	log.Printf("Done generating the Terraform manifests")
	return nil
}
...
func main() {
	...
	if len(ResourceTemplates) == 0 {
		if err := genTerraformCode(src, dst, excludeList...); err != nil {
			panic(err)
		}
	}

	if reverse {
		if err := getTerraformTemplates(dst, src, excludeList...); err != nil {
			panic(err)
		}
	}
}
