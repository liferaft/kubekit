...
var defaultMasterNodePool = NodePool{
	...
	Nodes: []Node{
		Node{
			...
			PublicDNS:  requiredValue + "master1.raw.test",
...
// Config defines the raw configuration parameters in the Cluster config file
type Config struct {
	clusterName            string
...
// NodePool defines the settings for group of instances on raw
...
// NewConfigFrom returns a new raw configuration from a map, usually from a
// cluster config file
