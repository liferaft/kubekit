...
func TestNew(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "new stacki with no env vars",
			args: args{
				envConfig: nil,
			},
			want: defaultVS([]string{"my_access_key", "my_secret_key", "my_session_token", "my_aws_region"}),
		},
		{
			name: "create stacki from nil config with nil string value",
			args: args{
				envConfig: nil,
			},
			want: modifiedVS("Username", nil, []string{"my_access_key", "my_secret_key", "my_session_token"}),
		},
	}
